...
def student(request):
    ...
    return render(request, template_name="crud/student.html", context={"title":"Student", "students":students})
...
def add_student(request):
    if request.method == "POST":
        ...
        st = Student.objects.create(name=name, email=email, age=age, classroom_id=class_id)
        sp= StudentProfile.objects.create(address=address, contact=contact, student = st)
        if pp:
            sp.profile_picture = pp
            sp.save()
        return redirect("crud_student")
    return render(request, template_name="crud/add_student.html", context={"title":"Add Student", "classes": ClassRoom.objects.all()})


def student_detail(request, id):
    std = Student.objects.get(id=id)
    return render(request, template_name="crud/detail.html", context={"title": "Student Detail", "student": std})
...
def student_update(request, id):
    ...
    if request.method == "POST":
        ...
        sp, _ = StudentProfile.objects.update_or_create(student=std, defaults={"address":address, "contact":contact})

        if pp:
            sp.profile_picture=pp
            sp.save()
        return redirect("student_detail", std.id)
    return render(request, template_name="crud/student_update.html", context={"title": "Student Update", "student": std})